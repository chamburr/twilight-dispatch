...
class State:
    ...
    def _members_get(self, key, key_id=None, name=None, first=None, second=None, predicate=None):
    ...
    def _members_get_all(self, key, key_id=None, name=None, first=None, second=None, predicate=None):
        results = []
        for match in self._members(key, key_id):
            keys = match.split(":")
            if name is None or keys[0] == str(name):
                if first is None or (len(keys) >= 1 and keys[1] == str(first)):
                    if second is None or (len(keys) >= 2 and keys[2] == str(second)):
                        if predicate is None or predicate(match) is True:
                            results.append(self._get(match))
        return results
    ...
    def _users(self):
        results = []
        user_ids = []
        for match in self._members("member"):
    ...
    @property
    def _emojis(self):
        results = self._members_get_all("emoji")
        emojis = []
        for result in results:
            guild = self._get_guild(self._key_first(result))
            if guild:
    ...
    def get_user(self, user_id):
        result = self._members_get("member", second=user_id)
        if result:
    ...
    def get_emoji(self, emoji_id):
        result = self._members_get("emoji", second=emoji_id)
        if result:
            guild = self._get_guild(self._key_first(result))
            if guild:
    ...
    def _get_message(self, msg_id):
        result = self._members_get("message", second=msg_id)
        if result:
            channel = self.get_channel(self._key_first(result))
            if channel:
    ...
    def _get_guild_channel(self, channel_id):
        result = self._members_get("channel", second=channel_id)
        if result:
    ...
    def parse_message_delete_bulk(self, data, old):
        ...
        if old:
            ...
            for old_message in old:
                channel = self.get_channel(int(old_message["channel_id"]))
                if channel:
    ...
    def parse_webhooks_update(self, data, old):
        channel = self._get_guild(int(data["channel_id"]))
        if channel:
        ...
        if guild:
            ...
            if member:
                ...
                if channel:
                    ...
                    old_channel = self.get_channel(old["channel_id"])
